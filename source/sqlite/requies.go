...
func GetAllRequres(in string, l int, res *[][]ReqRes) {
	if in != "" {
		re, _ := GetRequres(in)
		for _, item := range re {
			...
			if existed == false {
				...
				GetAllRequres(item.Name, l+1, res)
			}
		}
	}
}

func GetRequres(in string) (res []ReqRes, err error) {
	db, err := sql.Open("sqlite3", "/var/cache/godnf/BaseOS/baseos-primary.sqlite")
	if err != nil {
